...
import me.n1ar4.jar.obfuscator.asm.StringVisitor;
...
public class StringTransformer {
    ...
    public static void transform(CustomClassLoader loader) {
        for (Map.Entry<String, String> entry : ObfEnv.classNameObfMapping.entrySet()) {
            ...
            logger.debug("字符串加密混淆进行中 {} -> {}", newClassPath.toAbsolutePath());
            ...
            try {
                TransformerUtil.transformClass(newClassPath, loader, StringVisitor::new);
            } catch (MethodTooLargeException ex) {
                // 字符串字面量换成"解密调用"同样会让方法变长;
                // 巨型 <clinit> 也吃不下。跳过该类,不让整包失败。
                logger.warn("跳过字符串加密(方法超 64KB 上限):{} - {}",
                        newName, ex.getMessage());
            } catch (Exception ex) {
                logger.error("transform error: {}", ex.toString());
                throw new IllegalStateException("string transform failed: " + newName, ex);
            }
        }
        logger.info("encrypt string finish");
    }
}
